...
public class RegulaController : ControllerBase
{
        ...
        IOptions<RegulaOptions> optionsAccessor)
    {
        ...
        {
            return BadRequest(new { error = "No image provided. Send multipart/form-data with 'image' or JSON with 'imageBase64'." });
        }

        var options = optionsAccessor.Value;
        var client = httpClientFactory.CreateClient("Regula");
        ...
        {
            return StatusCode((int)response.StatusCode, new { error = "Regula detect request failed.", details = content });
        }
        ...
        IOptions<RegulaOptions> optionsAccessor)
    {
        ...
        if (!string.IsNullOrWhiteSpace(body.TransactionId))
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                return StatusCode((int)response.StatusCode, new { error = "Regula liveness request failed.", details = content });
            }
        ...
        {
            return BadRequest(new { error = "Provide 'transactionId' or an array of 'frames' (base64 images)." });
        }
        ...
        {
            return StatusCode((int)framesResponse.StatusCode, new { error = "Regula liveness frames request failed.", details = framesContent });
        }
        ...
        IOptions<RegulaOptions> optionsAccessor)
    {
        ...
        {
            return BadRequest(new { error = "Provide two images. Send multipart/form-data with 'image1' and 'image2' or JSON with 'imageBase64_1' and 'imageBase64_2'." });
        }
        ...
        if (!response.IsSuccessStatusCode)
        {
            return StatusCode((int)response.StatusCode, new { error = "Regula match request failed.", details = content });
        }
                ...
                foreach (var detection in detections.EnumerateArray())
                {
                    ...
                    foreach (var detail in details.EnumerateArray())
                    {
                        ...
                        if (detail.TryGetProperty("value", out var valueElement))
                        {
                            ...
                            //{
                            //    age = TryReadAge(valueElement);
                            //}
                            //else if (string.Equals(name, "Sex", StringComparison.OrdinalIgnoreCase))
                            //{
                            //    gender = TryReadGender(valueElement);
                            //}
                        }
                    }
