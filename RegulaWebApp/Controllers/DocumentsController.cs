...
public class DocumentsController : ControllerBase
{
        ...
        IOptions<DocROptions> optionsAccessor)
    {
        ...
        if (request.Images.Count == 0)
        {
            return BadRequest(new { error = "Provide document images. Send multipart/form-data with 'images'." });
        }

        if (string.IsNullOrWhiteSpace(request.LivePortraitBase64))
        {
            return BadRequest(new { error = "Provide a live portrait. Send form field 'livePortrait' with base64 or JSON 'livePortraitBase64'." });
        }

        var options = optionsAccessor.Value;
        var faceApiConfig = new DocRFaceApiConfig(options.FaceApiUrl, options.FaceApiMode, 0.85);
        ...
        return Ok(new { similarityPercent });
    }
    ...
    private static object BuildDocRSummary(string json)
    {
        try
        {
            ...
            var overallStatus = validity is { } v && (v.valid.Count > 0 || v.invalid.Count > 0)
                ? (v.invalid.Count == 0 ? "valid" : "invalid")
